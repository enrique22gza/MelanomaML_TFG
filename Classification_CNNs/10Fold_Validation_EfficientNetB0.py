...
# Cross-validation para EfficientNetB0
...
for train_index, val_index in kf.split(data['filename'], data['class']):
    ...
    model_checkpoint = ModelCheckpoint(f'efficientnet_model_fold_{fold_no}.keras', save_best_only=True, monitor='val_loss')
    ...
    results_efficientnet.append(model_efficientnet.evaluate(val_gen_split))
    fold_no += 1

# Resultados
print(f'Resultados de {num_folds}-Fold Cross Validation para EfficientNetB0:')
for i, result in enumerate(results_efficientnet):
